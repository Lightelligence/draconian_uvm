...
class InstancenameSchema(filters.LineListener):
    ...
    thisdict = {"uvm_analysis_port": "ap", "uvm_analysis_imp": "imp", "uvm_event": "event"}
        ...
        if match:
            ...
            if classname in self.thisdict:
                if not instancename.endswith(self.thisdict[classname]):
                    self.error(
                        line_no, line,
                        "Instance name '{}' not ending with '{}'. Recommend using suffix '{}' as instance name for '{}'"
                        .format(instancename, self.thisdict[classname], self.thisdict[classname], classname))
