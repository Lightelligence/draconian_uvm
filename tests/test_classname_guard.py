...
class ClassnameGuardTestCase(test.TestCase):
    ...
    def test_pkgname_match_dirname(self):
        ...
        with mock.patch.object(self.cut, "error", autospec=True):
            lb = lbc("level1/level2/sys_env/sys_env_pkg.sv", content, parent=None, gc=None, restrictions=self.build_restriction_filter(self.cut))
    ...
    def test_pkgname_not_match_dirname(self):
        ...
        """)
        with mock.patch.object(self.cut, "error", autospec=True):
            lb = lbc("level1/level2/sys_env/sys_pkg.sv", content, parent=None, gc=None, restrictions=self.build_restriction_filter(self.cut))
            iut = self.get_listener(lb, self.cut)
    ...
    def test_component_name_match_expectation(self):
        ...
        """)
        with mock.patch.object(self.cut, "error", autospec=True):
            lb = lbc("level1/level2/sys_env/sys_env_sb.sv", content, parent=None, gc=None, restrictions=self.build_restriction_filter(self.cut))
            iut = self.get_listener(lb, self.cut)
            iut.error.assert_not_called()
    ...
    def test_component_name_not_match_expectation(self):
        """include file name should match with directory name - env."""
        content = StringIO("""
        class sys_env_sb_c extends uvm_component;
        """)
        with mock.patch.object(self.cut, "error", autospec=True):
            lb = lbc("level1/level2/sys_env/sys_env_sb.sv", content, parent=None, gc=None, restrictions=self.build_restriction_filter(self.cut))
            iut = self.get_listener(lb, self.cut)
